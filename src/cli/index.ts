...
function getConfig(): any {
    ...
    return { agent: { model: 'auto' }, swarm: { ollama_url: 'https://platform.gstdtoken.com' } };
}
...
program
    ...
    .option('--swarm-url <url>', 'Swarm/Platform URL', process.env.GSTD_SWARM_URL || 'https://platform.gstdtoken.com')
    ...
    .action(async () => {
        ...
        if (!fs.existsSync(configPath)) {
            fs.writeFileSync(configPath, JSON.stringify({
                ...
                swarm: { enabled: true, contribute: true, ollama_url: 'https://platform.gstdtoken.com' },
    ...
    .action(async () => {
        ...
        const checks: Array<{ name: string; check: () => Promise<boolean> | boolean; fix?: string }> = [
            ...
            },
            {
                ...
                check: async () => { const r = await fetch('https://platform.gstdtoken.com/api/v1/health'); return r.ok; },
...
swarmCmd.command('status').description('Show swarm network status').action(async () => {
    ...
    try {
        const resp = await fetch('https://platform.gstdtoken.com/api/v1/network/stats');
        const data: any = await resp.json();

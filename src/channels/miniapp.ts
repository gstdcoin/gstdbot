...
export class MobileNodeManager {
    private sessions: Map<number, MobileNodeSession> = new Map();
    ...
    registerRoutes(app: Express): void {
        ...
        const validateTMA = (req: Request, res: Response, next: NextFunction) => {
            ...
            (req as any).tmaUser = user;
            next();
        };

        // ── Activate Mobile Node ──
        app.post('/tma/node/activate', validateTMA, async (req: Request, res: Response) => {
            const user = (req as any).tmaUser;
        ...
        // ── Node Status ──
        app.get('/tma/node/status', validateTMA, (req: Request, res: Response) => {
            const user = (req as any).tmaUser;
        ...
        app.post('/tma/node/heartbeat', validateTMA, async (req: Request, res: Response) => {
            const user = (req as any).tmaUser;
            const session = this.sessions.get(user.id);
            ...
            let batteryWarning: string | null = null;
        ...
        // ── Link TON Wallet ──
        app.post('/tma/node/link-wallet', validateTMA, async (req: Request, res: Response) => {
            const user = (req as any).tmaUser;
        ...
        // ── Stop Node ──
        app.post('/tma/node/stop', validateTMA, async (req: Request, res: Response) => {
            const user = (req as any).tmaUser;
        ...
        // ── Complete Task ──
        app.post('/tma/node/task/complete', validateTMA, async (req: Request, res: Response) => {
            const user = (req as any).tmaUser;
        ...
        // ── Claim Rewards ──
        app.post('/tma/node/claim', validateTMA, async (req: Request, res: Response) => {
            const user = (req as any).tmaUser;
            const session = this.sessions.get(user.id);
        ...
        app.get('/tma/network/stats', (req: Request, res: Response) => {
        ...
        app.get('/tma/node/linked-wallet', validateTMA, async (req: Request, res: Response) => {
            const user = (req as any).tmaUser;
            try {
    ...
    private sanitizeDeviceInfo(raw: any): DeviceResources {
        if (!raw || typeof raw !== 'object') return this.defaultDeviceResources();
        return {
    ...
    private defaultDeviceResources(): DeviceResources {
        return {
    ...
    private async activateNode(telegramId: number, info: {
        username: string;
        firstName: string;
        walletAddress?: string;
        device?: DeviceResources;
    }): Promise<MobileNodeSession> {
        // Check if already active
        const existing = this.sessions.get(telegramId);
        if (existing?.isActive) {
            // Update device resources on re-activate
            if (info.device) existing.device = info.device;
        ...
        const interval = setInterval(async () => {
            ...
            if (Date.now() - s.lastHeartbeat > 120000) {
                logActivity(`Mobile node ${s.nodeId} timed out`, 'warn');
        ...
        logActivity(`Mobile node activated: ${session.nodeId} (${info.firstName})`, 'success');
        return session;
    }

    private async deactivateNode(telegramId: number): Promise<void> {
    ...
    private async pollTask(session: MobileNodeSession): Promise<any | null> {
        try {
    ...
    private validateInitData(initData: string): any | null {
        try {
    ...
    private getHourlyRate(tier: string): number {
    ...
    private formatUptime(minutes: number): string {
    ...
    private getNextRewardTime(session: MobileNodeSession): number {

...
exports.refreshDynamicPricing = refreshDynamicPricing;
...
class NeuralRouter {
    ...
    async routeSmartMix(tier, messages) {
        ...
        const expertPromises = experts.map(model => this.callSingleGroq(model, messages, 1500).catch(err => {
...
async function refreshDynamicPricing() {
    ...
    if (price <= 0)
        return;
    for (const key of Object.keys(exports.SMARTMIX_TIERS)) {
        const tier = exports.SMARTMIX_TIERS[key];
        if (tier.costUsd > 0) {
            tier.cost = parseFloat((tier.costUsd / price).toFixed(1));
        }
    }
    console.log(`[Pricing] Updated: 1 GSTD = $${price.toFixed(8)} | Council=${exports.SMARTMIX_TIERS.standard.cost} | Panel=${exports.SMARTMIX_TIERS.pro.cost} | Swarm=${exports.SMARTMIX_TIERS.ultra.cost}`);
}

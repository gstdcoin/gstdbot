...
function makeKnowledgeKey(question) {
...
class TelegramChannel {
    ...
    constructor(config) {
        ...
        this.router = new router_js_1.NeuralRouter(config.swarmUrl, config.cocoonEnabled);
        this.guardian = new guardian_js_1.CommunityGuardian({
...
QUALITY BAR: Your answer must be the BEST the user has ever received from any AI. If it wouldn't satisfy a demanding expert in the field, iterate before sending.`;
            ...
            try {
                ...
                const mixTier = ctx.session.mixTier;
                if (mixTier && mixTier !== 'free') {
                    const tierInfo = router_js_1.SMARTMIX_TIERS[mixTier] || router_js_1.SMARTMIX_TIERS.free;
                    ...
                    const resultTierInfo = router_js_1.SMARTMIX_TIERS[mixResult.tier] || router_js_1.SMARTMIX_TIERS.free;
    ...
    // ── SmartMix Menu ──
    async handleSmartMixMenu(ctx, lang) {
    ...
    async handleSponsorDeepLink(ctx, lang, signalId, starsCost) {
        ...
        // Send Stars invoice
        try {
            await ctx.api.sendInvoice(ctx.chat.id, title, desc, `gstd_sponsor_${signalId}_${ctx.from.id}_${Date.now()}`, 'XTR', [{ label: title, amount: starsCost }]);
    ...
    async handleBalance(ctx, lang) {
        try {
            const data = await this.apiCall(`/api/v1/telegram/bot/balance?telegram_id=${ctx.from.id}`);
            const costPerPro = router_js_1.SMARTMIX_TIERS.standard.cost;
            ...
            if (lang === 'ru') {
                ...
                msg += `\n\n<i>🆓 Free model always available\n⚡ Pro = ${costPerPro.toFixed(1)} GSTD/request ($${router_js_1.SMARTMIX_TIERS.standard.costUsd})</i>`;
            }
    ...
    async sendHelp(ctx) {
        ...
        if (isPrivate) {
            ...
            await ctx.reply(`🐝 <b>GSTD Bot</b>\n\nTag @${this.bot.botInfo?.username} to ask me anything!`, { parse_mode: 'HTML' });
        }
    }
    async start() {
        ...
        this.bot.start({
            onStart: (botInfo) => {
                ...
                this.guardian.config.botUsername = botInfo.username;
    ...
    async sendFormattedReply(ctx, html, isGroup) {
        const MAX_LEN = 4000;

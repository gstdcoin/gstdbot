    ...
    async register() {
        try {
            ...
            if (result?.id || result?.node_id || result?.ok) {
                ...
                (0, server_js_1.logActivity)(`Registered with platform (ID: ${this.config.nodeId.slice(0, 12)}...)`, 'success');
            }
            else {
                (0, server_js_1.logActivity)('Platform registration pending — will retry on heartbeat', 'warn');
            }
        }
        catch (e) {
            (0, server_js_1.logActivity)('Platform registration error: ' + (e.message || 'network'), 'error');
        }
    }
    // ─── Heartbeat ───────────────────────────────────────────────
    async heartbeat() {
        try {
            ...
            const payload = {
                ...
                node_id: this.config.nodeId,
                status: 'online',
                battery: 100 - Math.round(load[0] * 100 / (0, os_1.cpus)().length), // CPU as "battery" (inverse)

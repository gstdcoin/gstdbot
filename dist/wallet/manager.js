...
class NodeWallet {
    ...
    stakingCache = { staked: 0, apy: 0, rewardsPending: 0 };
    stakingSyncTimer = null;
    async syncStakingData() {
        if (!this.wallet)
            return;
        try {
            const resp = await fetch(`${this.config.swarm.apiUrl}/api/v1/staking/info?wallet=${encodeURIComponent(this.wallet.address)}`, { signal: AbortSignal.timeout(5000) }).catch(() => null);
            if (resp?.ok) {
